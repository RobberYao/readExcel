...
public class ExcelUtils {
  ...
  /**
   * 解决思路：采用Apache的POI的API来操作Excel，读取内容后保存到List中，
   * 再将List转Json（推荐Linked，增删快，与Excel表顺序保持一致）
   * 
   ...
   * List 一个sheet表的一行数据就是一个 Map 一行中的一列，就把当前列头为key，列值为value存到该列的Map中.
   *
   * @param file SSM框架下用户上传的Excel文件
   * @return Map 一个线性HashMap，以Excel的sheet表顺序，并以sheet表明作为key，sheet表转换json后的字符串作为value
   * @throws IOException Signals that an I/O exception has occurred.
   */
  public static LinkedHashMap<String, String> excel2json(File file) throws IOException {
